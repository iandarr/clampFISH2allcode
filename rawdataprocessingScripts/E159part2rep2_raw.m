...
diskVolumeName='/Volumes/IAND_08/';
cellposeCommandsFileAllScansDirectory='/Volumes/IAND_08/rawData/E159part2rep2_scan/scan';
...
%% create a .txt file with the commands to call cellpose for NUCLEI in scanID=3
% AND save the existing cytoplasmic masks in a subfolder 'cytoMaskSave'

scanIDsList=3;


cellposeModel='nuclei'; % either 'cyto' or 'nuclei'. Make sure cellposeDiam
cytoRGBindex=2; % 2 = green in RGB image
nucleiRGBindex=3; % 3 = blue in RGB image
...
for iScanID=1:length(scanIDsList)
    ...
    for iSubregion=1:numSubregion
        ...
        %% rename the existing Merged_Rcyto_YFP_DAPI_cp_masks.tif and Merged_Rcyto_YFP_DAPI_cp_outlines.txt into a subfolder so it doesn't get overwritten
        subregionAbsoluteDir=subregionAbsoluteDirList{iSubregion};
        if ~isfolder(fullfile(subregionAbsoluteDir,filesep,'cytoMaskSave',filesep))
            mkdir(fullfile(subregionAbsoluteDir,filesep,'cytoMaskSave',filesep))
        end
        copyfile(fullfile(subregionAbsoluteDir,filesep,'Merged_Rcyto_YFP_DAPI_cp_masks.tif'),fullfile(subregionAbsoluteDir,filesep,'cytoMaskSave',filesep,'Merged_Rcyto_YFP_DAPI_cp_mask.tif'))
        copyfile(fullfile(subregionAbsoluteDir,filesep,'Merged_Rcyto_YFP_DAPI_cp_outlines.txt'),fullfile(subregionAbsoluteDir,filesep,'cytoMaskSave',filesep,'Merged_Rcyto_YFP_DAPI_cp_outlines.txt'))
        
        % get the cellpose command string
        subregionCellposeStr=sprintf('python -m cellpose --dir %*s --img_filter %s --pretrained_model %s %s %s --fast_mode --no_npy --save_tif --diameter %i',...
                                              dirStrPad,     subregionAbsoluteDir,   imgNameOfMerged,   cellposeModel, chan_str, chan2_str,               cellposeDiameter);
        % file for each well
        fprintf(fid,subregionCellposeStr);
